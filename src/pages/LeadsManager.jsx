...
import { motion, AnimatePresence } from 'framer-motion';
 ...
 />
 <motion.div
 className={`relative z-10 bg-slate-200 dark:bg-[#1e293b] border border-slate-300 dark:border-slate-600 rounded-2xl w-full ${maxWidth} max-h-[calc(100vh-24px)] sm:max-h-[90vh] flex flex-col overflow-hidden`}
 ...
 {/* Sticky Header */}
 <div className="flex justify-between items-center p-4 sm:p-5 border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-200 dark:bg-[#1e293b] z-20 flex-shrink-0">
 <h3 className="text-lg sm:text-xl font-bold text-black dark:text-white truncate pr-4">{title}</h3>
 <button onClick={onClose} className="text-black dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1.5 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors flex-shrink-0">
 <X size={20} />
 </button>
 </div>
 ...
 <label className="block text-[11px] sm:text-xs font-semibold text-black dark:text-slate-400 mb-1.5 uppercase tracking-wider">{label}</label>
 {children}
 </div>
 );
 ...
 <input
 className={`w-full bg-slate-100 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 sm:py-2.5 text-sm sm:text-base text-black dark:text-slate-200 placeholder-slate-500
 ...
 className={`w-full bg-slate-100 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 sm:py-2.5 text-sm sm:text-base text-black dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40 transition-all flex-1 min-w-[80px] ${className}`}
 autoFocus
 />
 <button
 type="button"
 ...
 className="flex items-center justify-center bg-slate-200 dark:bg-slate-800 text-black dark:text-slate-400 hover:text-rose-400 hover:bg-slate-300 dark:hover:bg-slate-700 rounded-lg border border-slate-300 dark:border-slate-600 transition-colors flex-shrink-0 p-2 sm:p-2.5"
 ...
 className={`w-full appearance-none bg-slate-100 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 sm:py-2.5 text-sm sm:text-base text-black dark:text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40 transition-all cursor-pointer ${className}`}
 >
 {placeholder !== undefined && <option value="" disabled hidden>{placeholder}</option>}
 {options.map(o => <option key={o} value={o}>{o}</option>)}
 <option value="__MANUAL__" className="font-bold text-emerald-400 bg-slate-200 dark:bg-slate-800">+ Add Manual / Other</option>
 </select>
 <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-black dark:text-slate-400 pointer-events-none" />
 </div>
 );
 };
 ...
 Other: 'bg-slate-500/10 text-black dark:text-slate-400 border-slate-500/20',
 };
 ...
 return (
 <div className="bg-[#E1E8F0] dark:bg-[#0f172a] min-h-screen w-full p-3 sm:p-4 lg:p-6 pt-20 sm:pt-24 lg:pt-24 space-y-4 sm:space-y-6 text-black dark:text-white text-base overflow-x-hidden">

 {/* ── HEADER ── */}
 <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-4 sm:p-5 lg:p-6 rounded-xl border border-slate-300 dark:border-slate-700 bg-[#F1F5F9] dark:bg-slate-900/50">
 <div>
 <h1 className="text-2xl sm:text-3xl font-bold text-black dark:text-white flex items-center gap-2 tracking-tight">
 <Users className="text-emerald-400" /> Leads Manager
 </h1>
 {/* <p className="text-sm sm:text-base text-black dark:text-slate-400 mt-1 sm:mt-1.5">View, search, edit, and manage all your customer travel inquiries.</p> */}
 </div>
 ...
 className="w-full bg-slate-100 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg pl-10 pr-4 py-2 sm:py-2.5 text-sm sm:text-base text-black dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30 transition-colors"
 />
 <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-black dark:text-slate-500" />
 </div>
 <button onClick={openAddModal} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg text-sm sm:text-base font-bold transition-colors whitespace-nowrap">
 ...
 <div className="bg-transparent lg:bg-[#F1F5F9] dark:lg:bg-slate-900/50 lg:border border-slate-300 dark:border-slate-700/50 rounded-xl overflow-hidden">
 <div className="w-full custom-scrollbar">
 <table className="w-full text-left text-sm sm:text-base text-black dark:text-slate-300 block lg:table">
 <thead className="text-xs uppercase tracking-wider text-black dark:text-slate-400 font-semibold bg-transparent border-b border-slate-200/50 dark:border-slate-700/50 hidden lg:table-header-group">
 <tr>
 ...
 <td colSpan="8" className="block lg:table-cell text-center py-12 text-black dark:text-slate-500">Loading your leads...</td>
 </tr>
 ) : filteredLeads.length === 0 ? (
 <tr className="block lg:table-row">
 <td colSpan="8" className="block lg:table-cell text-center py-12 text-black dark:text-slate-500">No leads found. Time to launch some marketing!</td>
 </tr>
 ) : (
 ...
 className="block lg:table-row bg-slate-200 dark:bg-[#132033] lg:bg-transparent border border-[#1e3a52] lg:border-none rounded-xl mb-4 lg:mb-0 p-3 lg:p-0 hover:bg-slate-300/40 dark:hover:bg-slate-800/40 transition-colors group">
 
 {/* Job ID */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none font-medium text-black dark:text-slate-200">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Job ID</span>
 <span className="text-right lg:text-left">LMN{lead.id}</span>
 </td>
 
 {/* Customer Info */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Customer</span>
 <div className="text-right lg:text-left">
 <div className="font-bold text-black dark:text-white text-sm sm:text-base leading-none mb-1.5">
 {lead.customerName || 'N/A'}
 </div>
 <div className="flex flex-col lg:items-start items-end gap-0.5 text-xs sm:text-sm text-black dark:text-slate-400">
 <span className="flex items-center gap-1"><Phone size={13} className="flex-shrink-0" /> {lead.phone}</span>
 {lead.email && <span className="text-[11px] sm:text-xs text-black dark:text-slate-500">{lead.email}</span>}
 </div>
 </div>
 </td>
 ...
 {/* Trip Details */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Trip Details</span>
 <div className="text-right lg:text-left">
 ...
 <div className="flex flex-col lg:items-start items-end gap-1 text-[11px] sm:text-xs text-black dark:text-slate-400">
 <span className="flex items-center gap-1.5"><Calendar size={13} className="flex-shrink-0"/> {lead.travelDates || 'TBD'}</span>
 <span className="text-black dark:text-slate-500 lg:pl-5">
 ...
 {/* Package & Budget */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Package / Budget</span>
 ...
 <span className="font-semibold text-black dark:text-slate-200 text-xs sm:text-sm">
 ...
 {/* Message & Notes */}
 <td className="flex flex-col lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none text-xs sm:text-sm text-black dark:text-slate-500">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider mb-1.5">Message / Notes</span>
 ...
 {/* Source */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Source</span>
 ...
 <span className="text-black dark:text-slate-600">—</span>
 )}
 {lead.campaign && (
 <div className="text-[10px] sm:text-xs text-black dark:text-slate-500 mt-1 truncate max-w-[120px]" title={lead.campaign}>
 {lead.campaign}
 </div>
 )}
 </div>
 ...
 {/* Date Added */}
 <td className="flex justify-between items-start lg:items-center lg:table-cell py-2.5 lg:py-4 px-3 lg:px-6 border-b border-slate-200/30 dark:border-slate-700/30 lg:border-none text-black dark:text-slate-400 text-xs sm:text-sm font-medium">
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Date Added</span>
 ...
 <span className="lg:hidden text-[11px] font-semibold text-black dark:text-slate-400 uppercase tracking-wider">Actions</span>
 <div className="flex items-center justify-end lg:justify-center gap-1.5 sm:gap-2">
 <button onClick={() => openViewModal(lead)}
 className="p-2 lg:p-1.5 text-blue-400 lg:text-black dark:lg:text-slate-400 bg-blue-500/10 lg:bg-transparent hover:text-blue-400 hover:bg-blue-900/30 rounded-lg transition-colors"
 title="View Lead">
 <Eye size={18} />
 </button>
 <button onClick={() => openHistoryModal(lead)}
 className="p-2 lg:p-1.5 text-purple-400 lg:text-black dark:lg:text-slate-400 bg-purple-500/10 lg:bg-transparent hover:text-purple-400 hover:bg-purple-900/30 rounded-lg transition-colors"
 title="Lead History">
 <History size={18} />
 </button>
 <button onClick={() => openEditModal(lead)}
 className="p-2 lg:p-1.5 text-yellow-400 lg:text-black dark:lg:text-slate-400 bg-yellow-500/10 lg:bg-transparent hover:text-yellow-400 hover:bg-yellow-900/30 rounded-lg transition-colors"
 title="Edit Lead">
 <Pencil size={18} />
 </button>
 <button onClick={() => deleteLead(lead.id)}
 className="p-2 lg:p-1.5 text-rose-400 lg:text-black dark:lg:text-slate-400 bg-rose-500/10 lg:bg-transparent hover:text-red-400 hover:bg-red-900/30 rounded-lg transition-colors"
 title="Delete Lead">
 <Trash2 size={18} />
 </button>
 </div>
 ...
 <div>
 <h4 className="text-sm sm:text-base font-bold text-black dark:text-slate-300 border-b border-slate-200/50 dark:border-slate-700/50 pb-2 mb-3 sm:mb-4 flex items-center gap-2">
 ...
 className="w-full bg-slate-100 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg pl-3 pr-10 py-2 sm:py-2.5 text-sm sm:text-base text-black dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/40 transition-all cursor-pointer [&::-webkit-calendar-picker-indicator]:hidden"
 />
 {/* Changed from <button> to <div> and added pointer-events-none */}
 <div className="absolute right-3 text-black dark:text-slate-400 pointer-events-none flex items-center justify-center">
 <Calendar size={18} />
 </div>
 </div>
 </Field>
 ...
 {/* Sticky Footer */}
 <div className="flex flex-col sm:flex-row gap-3 pt-4 sm:pt-5 border-t border-slate-200/50 dark:border-slate-700/50 mt-6 sticky -bottom-6 bg-slate-200 dark:bg-[#1e293b] pb-6 sm:pb-0 z-20">
 <button onClick={closeModal} className="w-full sm:flex-1 py-2.5 sm:py-3 rounded-lg border border-slate-300 dark:border-slate-600 text-black dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 text-sm sm:text-base font-semibold transition-colors order-2 sm:order-1">
 Cancel
 </button>
 <button onClick={saveLead} disabled={!leadForm.customerName.trim()}
 className="w-full sm:flex-1 py-2.5 sm:py-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:text-black dark:disabled:text-slate-500 text-white text-sm sm:text-base font-bold transition-colors flex items-center justify-center gap-2 order-1 sm:order-2">
 ...
 <div className="bg-slate-300/50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
 <h4 className="text-emerald-400 font-bold mb-3 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2">
 <Users size={16} /> Customer Information
 </h4>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Name</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.customerName || 'N/A'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Phone</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.phone || 'N/A'}</p>
 </div>
 <div className="col-span-2">
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Email</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.email || 'N/A'}</p>
 </div>
 </div>
 </div>

 <div className="bg-slate-300/50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
 <h4 className="text-blue-400 font-bold mb-3 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2">
 <PlaneTakeoff size={16} /> Trip Requirements
 </h4>
 <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Destination</p>
 <p className="font-medium text-black dark:text-white capitalize">{viewingLead.destination || 'N/A'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Travel Dates</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.travelDates || 'TBD'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Adults</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.pax || 'TBD'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Children</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.noOfChildren || viewingLead.childrenPax || '0'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Budget</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.budget || 'TBD'}</p>
 </div>
 <div className="col-span-2 sm:col-span-1">
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Package Type</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.packageType || 'Custom / Flexible'}</p>
 </div>
 </div>
 </div>

 <div className="bg-slate-300/50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
 <h4 className="text-purple-400 font-bold mb-3 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2">
 <MessageSquare size={16} /> Additional Information & Marketing
 </h4>
 <div className="grid grid-cols-2 gap-4 mb-4">
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Source Platform</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.platform || 'N/A'}</p>
 </div>
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Campaign</p>
 <p className="font-medium text-black dark:text-white">{viewingLead.campaign || 'N/A'}</p>
 </div>
 </div>
 <div className="space-y-3">
 <div>
 <p className="text-xs text-black dark:text-slate-500 uppercase font-semibold">Message from Lead</p>
 <p className="p-2 bg-slate-100 dark:bg-[#0f172a] rounded border border-slate-200 dark:border-slate-700 text-sm">{viewingLead.leadMessage || 'No message provided.'}</p>
 </div>
 ...
 <button onClick={closeViewModal} className="px-6 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-300 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-black dark:text-white font-semibold transition-colors">
 Close Window
 </button>
 </div>
 </div>
 ...
 <div className="bg-slate-300/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 space-y-1.5">
 <div className="flex flex-wrap items-center gap-2">
 <span className="text-xs uppercase tracking-wider text-black dark:text-slate-500 font-semibold">Current Stage</span>
 ...
 <span className="text-xs text-black dark:text-slate-400">Handled By: <span className="text-black dark:text-slate-200 font-semibold">{historyLead.assignedTo}</span></span>
 )}
 </div>
 <p className="text-sm font-bold text-black dark:text-slate-100">{historyLead.leadResponse || historyLead.status || 'N/A'}</p>
 <div className="text-xs text-black dark:text-slate-400 space-y-0.5">
 <p>Lead Created: <span className="text-black dark:text-slate-300">{leadCreated || 'N/A'}</span></p>
 <p>Lead Updated: <span className="text-black dark:text-slate-300">{leadUpdated || 'N/A'}</span></p>
 </div>
 </div>

 {/* Chronological Timeline — diamond markers, oldest → newest */}
 <div>
 <h4 className="text-sm font-bold text-black dark:text-slate-300 mb-1 flex items-center gap-2">
 <History size={16} className="text--400" /> Full Journey
 </h4>
 <div className="h-px bg-emerald-500/40 mb-4" />
 {timeline.length === 0 ? (
 <p className="text-sm text-black dark:text-slate-500 italic">No activity recorded yet.</p>
 ) : (
 <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-2 space-y-5">
 {timeline.map((log, idx) => {
 const isCurrent = idx === timeline.length - 1;
 const stageColor = STAGE_CONFIG.find(s => s.key === log.stage)?.color?.split(' ')[0] || 'text-black dark:text-slate-300';
 const body = (
 <>
 <p className="text-xs text-black dark:text-slate-500 mb-0.5">{log.date}</p>
 <p className={`text-sm font-bold ${stageColor}`}>{log.title}</p>
 {log.parts && log.parts.length > 0 && (
 <p className="text-xs text-black dark:text-slate-400 mt-1 leading-relaxed">
 {log.parts.map((p, pi) => (
 <span key={pi}>
 {pi > 0 && ' | '}
 {p.label && <span className="text-black dark:text-slate-400">{p.label}{p.value ? ': ' : ''}</span>}
 ...
 <div>
 <h4 className="text-sm font-bold text-black dark:text-slate-300 mb-3 flex items-center gap-2 border-t border-slate-200/50 dark:border-slate-700/50 pt-5">
 <ClipboardList size={16} className="text-black dark:text-slate-400" /> Complete Record by Stage
 </h4>
 ...
 return (
 <div key={stage.key} className={`rounded-xl border ${hasData ? 'border-slate-200 dark:border-slate-700' : 'border-slate-200 dark:border-slate-800'} overflow-hidden`}>
 <button
 type="button"
 onClick={() => hasData && setExpandedStage(isOpen ? null : stage.key)}
 className={`w-full flex items-center justify-between px-4 py-3 text-left transition-colors ${hasData ? 'bg-slate-300/50 dark:bg-slate-800/50 hover:bg-slate-300 dark:hover:bg-slate-800 cursor-pointer' : 'bg-slate-200/30 dark:bg-slate-900/30 cursor-default'}`}
 >
 <span className={`flex items-center gap-2 text-sm font-bold ${hasData ? stage.color.split(' ')[0] : 'text-black dark:text-slate-600'}`}>
 <StageIcon size={15} /> {stage.label}
 </span>
 {hasData ? (
 <span className="flex items-center gap-2 text-xs text-black dark:text-slate-400">
 {populated.length} field{populated.length > 1 ? 's' : ''} recorded
 <ChevronRight size={14} className={`transition-transform ${isOpen ? 'rotate-90' : ''}`} />
 </span>
 ) : (
 <span className="text-xs text-black dark:text-slate-600 italic">Not reached yet</span>
 )}
 </button>
 {hasData && isOpen && (
 <div className="px-4 py-3 bg-slate-100 dark:bg-[#0f172a] grid grid-cols-1 sm:grid-cols-2 gap-3 border-t border-slate-200/50 dark:border-slate-700/50">
 {populated.map(([field, label]) => (
 <div key={field}>
 <p className="text-[10px] text-black dark:text-slate-500 uppercase font-semibold tracking-wider">{label}</p>
 <p className="text-sm text-black dark:text-slate-200 break-words">{String(enrichedHistoryLead[field])}</p>
 </div>
 ))}
 </div>
 )}
 ...
 <div className="flex justify-end pt-2">
 <button onClick={closeHistoryModal} className="px-6 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-300 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-black dark:text-white font-semibold transition-colors">
 Close Window
 </button>
 </div>
 </div>
