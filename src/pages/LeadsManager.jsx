...
const LeadsManager = () => {
    ...
    const [leads, setLeads] = useState(() => {
        try {
            const cached = sessionStorage.getItem('leadsManagerData');
    ...
    const [isLoading, setIsLoading] = useState(() => !sessionStorage.getItem('leadsManagerData'));
    ...
    // ── FETCH LEADS & CAMPAIGNS & BACKGROUND POLLING ──────────────────────────
    useEffect(() => {
        const fetchData = async (isSilent = false) => {
            try {
                ...
                if (leadsRes.ok) {
                    ...
                    sessionStorage.setItem('leadsManagerData', JSON.stringify(leadsData));
                }
        ...
        const hasCachedData = !!sessionStorage.getItem('leadsManagerData');
        fetchData(hasCachedData);

        // 3. Set up periodic background polling to refresh data silently (every 30 seconds)
        const backgroundPoll = setInterval(() => {
            fetchData(true); // true = silent
        }, 30000); 
    ...
    const saveLead = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const updatedLeads = isEditing ? leads.map(l => l.id === editingId ? saved : l) : [saved, ...leads];
                setLeads(updatedLeads);
                sessionStorage.setItem('leadsManagerData', JSON.stringify(updatedLeads));
